...
class GateioUnifiedExchange(UnifiedCompositeExchange):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise ExchangeRestError(500, f"Gate.io initialization failed: {str(e)}")
    ...
    async def _load_symbols_info(self) -> None:
        ...
        if not self._public_rest:
            raise ExchangeRestError(500, "Public REST client not initialized")
    ...
    async def _validate_private_access(self) -> None:
        ...
        if not self._private_rest:
            raise ExchangeRestError(500, "Private REST client not available")
    ...
    def symbols_info(self) -> SymbolsInfo:
        ...
        if not self._symbols_info:
            raise ExchangeRestError(500, "Symbols info not loaded")
    ...
    async def get_klines(self, 
        ...
        if not self._public_rest:
            raise ExchangeRestError(500, "Public REST client not available")
    ...
    async def add_symbols(self, symbols: List[Symbol]) -> None:
        ...
        try:
            if self._public_ws:
                await self._public_ws.add_symbols(symbols)
            ...
            raise ExchangeRestError(500, f"Add symbols failed: {str(e)}")
    
    async def remove_symbols(self, symbols: List[Symbol]) -> None:
        """Remove symbols from market data streaming."""
        try:
            if self._public_ws:
                await self._public_ws.remove_symbols(symbols)
            ...
            raise ExchangeRestError(500, f"Remove symbols failed: {str(e)}")
    ...
    async def get_open_orders(self, symbol: Optional[Symbol] = None) -> Dict[Symbol, List[Order]]:
        ...
        try:
            ...
            raise ExchangeRestError(500, f"Open orders fetch failed: {str(e)}")
        ...
        try:
            with LoggingTimer(self.logger, "place_limit_order") as timer:
                order = await self._private_rest.place_order(
                    symbol, side, OrderType.LIMIT, quantity, price, time_in_force=time_in_force, **kwargs
                )
            ...
            self.logger.error("Failed to place limit order",
                            ...
                            price=price,
                            error=str(e))
            raise ExchangeRestError(500, f"Limit order placement failed: {str(e)}")
    ...
    async def place_market_order(self,
        ...
        try:
            with LoggingTimer(self.logger, "place_market_order") as timer:
                # TODO: for market buy/sell need to pass some of one quantity or quote quantity
                # depending on exchange requirements, may not work
                order = await self._private_rest.place_order(
                    symbol, side, OrderType.MARKET, quantity, **kwargs
                )
            ...
            self.logger.error("Failed to place market order",
                            ...
                            quantity=quantity,
                            error=str(e))
            raise ExchangeRestError(500, f"Market order placement failed: {str(e)}")
    ...
    async def cancel_order(self, symbol: Symbol, order_id: OrderId) -> bool:
        ...
        try:
            with LoggingTimer(self.logger, "cancel_order") as timer:
                success = await self._private_rest.cancel_order(symbol, order_id)
            ...
            self.logger.error("Failed to cancel order",
                            ...
                            order_id=order_id,
                            error=str(e))
            raise ExchangeRestError(500, f"Order cancellation failed: {str(e)}")
    ...
    async def cancel_all_orders(self, symbol: Optional[Symbol] = None) -> List[bool]:
        ...
        try:
            ...
            raise ExchangeRestError(500, f"Bulk cancellation failed: {str(e)}")
    ...
    async def get_order_history(self, 
        ...
        try:
            ...
            raise ExchangeRestError(500, f"Order history fetch failed: {str(e)}")
    ...
    async def place_multiple_orders(self, orders: List[Dict[str, Any]]) -> List[Order]:
        ...
        try:
            ...
            self.logger.error("Failed to place multiple orders", 
                            ...
                            error=str(e))
            raise ExchangeRestError(500, f"Batch order placement failed: {str(e)}")
    ...
    async def cancel_multiple_orders(self, 
        ...
        try:
            ...
            self.logger.error("Failed to cancel multiple orders",
                            ...
                            error=str(e))
            raise ExchangeRestError(500, f"Batch cancellation failed: {str(e)}")
    ...
    async def withdraw(self, request: WithdrawalRequest) -> WithdrawalResponse:
        ...
        try:
            ...
            raise ExchangeRestError(500, f"Withdrawal submission failed: {str(e)}")
    ...
    async def cancel_withdrawal(self, withdrawal_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error("Failed to cancel withdrawal", 
                            ...
                            error=str(e))
            raise ExchangeRestError(500, f"Withdrawal cancellation failed: {str(e)}")
    ...
    async def get_withdrawal_status(self, withdrawal_id: str) -> WithdrawalResponse:
        ...
        try:
            ...
            self.logger.error("Failed to get withdrawal status",
                            withdrawal_id=withdrawal_id,
                            error=str(e))
            raise ExchangeRestError(500, f"Withdrawal status query failed: {str(e)}")
    ...
    async def get_withdrawal_history(self,
        ...
        try:
            ...
            raise ExchangeRestError(500, f"Withdrawal history fetch failed: {str(e)}")
    ...
    async def get_withdrawal_limits(self,
        ...
        try:
            ...
            self.logger.error("Failed to get withdrawal limits",
                            ...
                            network=network,
                            error=str(e))
            raise ExchangeRestError(500, f"Withdrawal limits query failed: {str(e)}")
