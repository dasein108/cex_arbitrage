...
class IcebergTask(BaseTradingTask[IcebergTaskContext]):
    ...
    async def _place_order(self):
        ...
        try:
            ...
            order_price = self._get_current_top_price() + vector_ticks * self.context.symbol_info.tick
            ...
            if order_quantity * order_price < self.context.symbol_info.min_quote_quantity:
                order_quantity = self.context.symbol_info.min_quote_quantity / order_price + 0.01
    ...
    async def _get_exchange_order(self) -> Order | None:
        """Get current order from exchange, track updates."""
        if self._curr_order:
            return await self._exchange.private.get_order(self._curr_order.symbol, self._curr_order.order_id)
        else:
            return None

    async def _should_cancel_order(self):
        if not self._curr_order:
            return True

        top_price = self._get_current_top_price()
        order_price = self._curr_order.price
        tick_difference = abs((order_price - top_price) / self.context.symbol_info.tick)
    ...
    async def _process_order_execution(self, order: Order):
        ...
        if is_order_done(order) and order.filled_quantity > 0:
