...
class MakerLimitConfig:
    ...
    position_size_usd: Decimal = field(default_factory=lambda: Decimal('100'))  # Base position size
    ...
    def calculate_max_position_size(self) -> Decimal:
        ...
        return self.position_size_usd * Decimal(str(self.max_position_size_multiplier))
    
    def is_within_delta_tolerance(self, net_delta: Decimal, position_size: Decimal) -> bool:
        """Check if net delta is within tolerance for delta neutrality"""
        if position_size == 0:
            return True
        delta_ratio = abs(net_delta / position_size)
        return delta_ratio <= Decimal(str(self.delta_tolerance_pct))
...
class MakerLimitRuntimeState:
    ...
    # Position tracking
    net_spot_position: Decimal = field(default_factory=lambda: Decimal('0'))
    net_futures_position: Decimal = field(default_factory=lambda: Decimal('0'))
    total_spot_volume: Decimal = field(default_factory=lambda: Decimal('0'))
    total_futures_volume: Decimal = field(default_factory=lambda: Decimal('0'))
    ...
    def reset_daily_counters(self):
        ...
        self.total_spot_volume = Decimal('0')
        self.total_futures_volume = Decimal('0')
