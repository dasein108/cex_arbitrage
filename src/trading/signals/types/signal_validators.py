...
class RDNSignalValidator:
    ...
    def validate_rdn_entry(self, df: pd.DataFrame, idx: int, position_size_usd: float = 1000,
        ...
        # 1. PROFIT VALIDATION - Simplified for now
        ...
        # Basic profit check - if spread is too small, skip
        if abs(current_spread) < self.min_profit_margin:
            return ValidationResult(
                is_valid=False,
                signal_type='HOLD',
                reason=f'Insufficient spread: {abs(current_spread):.3f}% < {self.min_profit_margin:.3f}%',
                confidence=90,
                expected_profit=abs(current_spread)
            )
        ...
        return ValidationResult(
            is_valid=True,
            signal_type='ENTER',
            reason=f'Valid entry: {abs(current_spread):.3f}% spread opportunity',
