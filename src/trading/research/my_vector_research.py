...
def add_execution_calculations(df: pd.DataFrame,
    ...
    # NEGATIVE = we get paid to enter (futures bid > spot ask)
...
def delta_neutral_backtest(
        df: pd.DataFrame,
        max_entry_cost_pct: float = 0.0,  # FIXED: Only enter if cost <= 0 (negative = paid)
    ...
    """
    Delta-neutral backtest - FIXED VERSION
    
    KEY FIX: Only enter when entry_cost is NEGATIVE (we get paid to enter!)
    
    Entry cost < 0 means: futures_bid > spot_ask
    → We receive more from selling futures than we pay for spot
    → Instant profit on entry!
    """
    trades = []
    ...
    for idx, row in df.iterrows():
        ...
        # ==========================================
        # ENTRY: When spread is NEGATIVE (we get paid)
        # ==========================================
        if position is None:
            ...
            # CRITICAL FIX: Only enter when cost is NEGATIVE or zero
            if entry_cost_pct <= max_entry_cost_pct:
                ...
                if opportunities_taken <= 10:
                    print(f"✅ Entry #{opportunities_taken}: Spread {entry_cost_pct:.4f}% (PAID to enter)")
    ...
    print(f"{'Opportunities seen:':<30} {opportunities_seen}")
    print(f"{'Opportunities taken:':<30} {opportunities_taken} ({opportunities_taken/opportunities_seen*100:.1f}%)")
...
def print_trade_summary(trades: list):
    ...
    if not trades:
        print("\n❌ No trades executed")
        print("\nThis means NO NEGATIVE SPREADS were found!")
        print("Try:")
        print("  1. Different time period")
        print("  2. Different symbol")
        print("  3. Different exchanges")
        return
    ...
    print(f"{'#':<4} {'Entry':<20} {'Exit':<20} "
          f"{'Entry Spread':<15} {'Exit Spread':<15} "
          f"{'P&L %':<10} {'Hrs':<6} {'Exit':<15}")
    print(f"{'-' * 160}")

    for i, t in enumerate(trades[:10], 1):
        marker = '✅' if t['net_pnl_pct'] > 0 else '❌'
        print(f"{i:<4} {str(t['entry_time']):<20} {str(t['exit_time']):<20} "
              f"{t['entry_spread_pct']:>14.4f}% {t['exit_spread_pct']:>14.4f}% "
...
async def main():
    ...
    print(f"{'=' * 80}")
    print(f"DELTA-NEUTRAL ARBITRAGE BACKTEST (FIXED)")
    print(f"{'=' * 80}")
    ...
    negative_spreads = (df['entry_cost_pct'] <= 0).sum()
    print(f"Negative spreads: {negative_spreads} / {len(df)} ({negative_spreads/len(df)*100:.1f}%)")
    print(f"Exit cost:  min={df['exit_cost_pct'].min():.4f}% "
          f"max={df['exit_cost_pct'].max():.4f}% "
          f"mean={df['exit_cost_pct'].mean():.4f}%\n")

    print("🚀 Running backtest (ONLY entering on NEGATIVE spreads)...\n")

    trades = delta_neutral_backtest(
        df,
        max_entry_cost_pct=0.0,  # FIXED: Only negative spreads
        min_profit_pct=0.1,
        max_hours=6,
        spot_fee=0.0005,
        fut_fee=0.0005
