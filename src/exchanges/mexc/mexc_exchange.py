...
class MexcExchange(BasePrivateExchangeInterface):
    ...
    def __init__(self,config: ExchangeConfig = None):
        ...
        self._ws_public: Optional[MexcWebsocketPublic] = None
        self._ws_private: Optional[MexcWebsocketPrivate] = None
        # Composition: Separate public and private cex implementations
        self._public_api: Optional[MexcPublicSpotRest] = MexcPublicSpotRest(config)
        self._private_api: Optional[MexcPrivateSpotRest] = MexcPrivateSpotRest(config) if self._config.has_credentials() else None
        ...
        self.logger.info(f"Initialized {self.exchange_name} exchange with HFT optimizations")
    ...
    def _get_websocket_state(self) -> ConnectionState:
        ...
        """
        if self._ws_public and self._ws_public.ws_client:
            return self._ws_public.ws_client.state
    ...
    def open_orders(self) -> Dict[Symbol, List[Order]]:
        ...
        # TODO: should be used ws event listener with fallback to rest to track orders
        return {}
    ...
    async def positions(self) -> Dict[Symbol, Position]:
        ...
        """
        # MEXC Spot Exchange - no futures positions
        raise NotImplementedError("Positions not supported on MEXC spot exchange")
