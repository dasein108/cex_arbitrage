...
class PublicExchangeInterface(ABC):
    """Abstract interface for public exchange operations"""
...
class MyExchangePublic(PublicExchangeInterface):
    async def get_orderbook(self, symbol: Symbol, limit: int = 100) -> OrderBook:
        # Implementation specific to MyExchange
        response = await self.client.get(f"/orderbook/{symbol}", params={"limit": limit})
...
class MyExchangePrivate(PrivateExchangeInterface):
    async def place_order(self, symbol: Symbol, side: Side, order_type: OrderType, 
        ...
        response = await self.client.post("/order", params=params)
