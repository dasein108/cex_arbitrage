"""Gate.io WebSocket Implementations"""

from .gateio_ws_public import GateioPublicSpotWebsocketBaseWebsocket
from .gateio_ws_private import GateioPrivateSpotWebsocket
from .gateio_ws_public_futures import GateioPublicFuturesWebsocketBaseWebsocket
from .gateio_ws_private_futures import GateioPrivateFuturesWebsocket


# Import registration to trigger auto-registration with factories
# from . import registration

__all__ = ['GateioPublicSpotWebsocketBaseWebsocket', 'GateioPrivateSpotWebsocket',
           'GateioPublicFuturesWebsocketBaseWebsocket',
           'GateioPrivateFuturesWebsocket']