...
class MexcWebsocketPrivate(BaseExchangePrivateWebsocketInterface):
    ...
    async def on_order_update(self, order: Order):
        ...
        self.logger.info(f"MEXC order update: {order.order_id} - {order.status} - {order.amount_filled}/{order.amount}")
    ...
    async def on_trade_update(self, trade: Trade):
        ...
        self.logger.info(f"MEXC trade executed: {trade.side.name} {trade.amount} at {trade.price} ({'maker' if trade.is_maker else 'taker'})")