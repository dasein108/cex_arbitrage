#!/usr/bin/env python3
"""
Simple Context State Test

Tests state and order_id management without exchange initialization.
"""
...
class MockTaskContext(IcebergTaskContext):
    """Mock context for testing."""
    pass


class MockTask(BaseTradingTask[MockTaskContext]):
    """Mock task for testing state management."""
    name = "MockTask"
    
    @property
    def context_class(self) -> Type[MockTaskContext]:
        return MockTaskContext
    
    async def _handle_executing(self):
        """Mock executing handler."""
        self.logger.debug("Mock executing handler called")
        # Simulate some work and complete
        await self.complete()
...
async def test_context_state():
    ...
    logger.info("🧪 Testing context state management")
    ...
    task = MockTask(config=config, logger=logger, context=context)
    ...
    logger.info("✅ State transition to IDLE verified", state=task.state.name)
    ...
    # Test 4: Execute once
    logger.info("=== Test 4: Execute Once ===")
    
    result = await task.execute_once()
    assert isinstance(result, TaskExecutionResult), f"Expected TaskExecutionResult, got {type(result)}"
    assert result.task_id == task.task_id, f"Expected {task.task_id}, got {result.task_id}"
    assert result.state == task.state, f"Expected {task.state}, got {result.state}"
    
    logger.info("✅ Execute once verified", 
    ...
    logger.info("=== Test 6: Context Serialization ===")
    ...
    new_context = MockTaskContext(symbol=symbol, side=Side.SELL)  # Dummy initial context
    new_task = MockTask(config=config, logger=logger, context=new_context)
    ...
    logger.info("🎉 All context state management tests passed!")
...
if __name__ == "__main__":
    print("🚀 Starting context state tests...")
    try:
        asyncio.run(test_context_state())
