...
class PrivateWebSocketExchangeFactory(BaseExchangeFactory):
    ...
    def register(cls, exchange_name: str, implementation_class: Type) -> None:
        ...
        logger.debug(f"Registered private WebSocket implementation for {exchange_key}: {implementation_class.__name__}")
    ...
    def inject(cls, exchange_name: str, config: Optional[ExchangeConfig] = None, 
        ...
        if exchange_key not in cls._implementations:
            ...
            raise ValueError(
                f"No private WebSocket implementation registered for {exchange_name}. "
        ...
        if cache_key in cls._instances:
            logger.debug(f"Returning cached private WebSocket instance for {exchange_key}")
        ...
        try:
            ...
            logger.info(f"Created and cached private WebSocket instance for {exchange_key}: {implementation_class.__name__}")
            return instance
            
        except Exception as e:
            logger.error(f"Failed to create private WebSocket instance for {exchange_key}: {e}")
            raise ValueError(f"Failed to create private WebSocket exchange {exchange_name}: {e}") from e
